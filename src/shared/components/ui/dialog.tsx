...
function useBodyScrollLock(enabled: boolean) {
  React.useLayoutEffect(() => {
    ...
    return () => {
      ...
      Object.assign(documentElement.style, originalHtmlStyles);
...
function Dialog({
  ...
  const shouldUseCustomScrollLock = currentOpen && modal === true;

  useBodyScrollLock(shouldUseCustomScrollLock);
...
function DialogOverlay({ className, ...props }: DialogPrimitive.Backdrop.Props) {
  return (
    <DialogPrimitive.Backdrop
      ...
      className={cn(
        'fixed inset-0 isolate z-50 bg-black/68 duration-200 supports-backdrop-filter:backdrop-blur-xs data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0',
        className,
      )}
      {...props}
    />
